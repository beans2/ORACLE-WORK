...
set serveroutput on
--class4 학생들의 번호, 이름, 오라클점수를 처리하는 PLSQL

declare
--학생의 정보를 저장할 record를 선언
	type stu_record is record(num number,name varchar2(30),oracle_score number(3));
--학생들의 정보를 저장할 table 선언
	type stu_table is table of stu_record
		index by binary_integer;
		
--테이블을 사용한 학생들의 정보를 처리할 변수 선언
